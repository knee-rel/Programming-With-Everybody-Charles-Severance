...
def build_plot_values(gdpinfo, gdpdata):
    ...
    for key,value in gdpdata.items():
      try:
        if (value != ""):
          if (int(key) <= gdpinfo['maxyear']) and (int(key) >= gdpinfo['min_year']):
...
def build_plot_dict(gdpinfo, country_list):
    ...
    """
    plot = {}
    plot_data = read_csv_as_nested_dict(gdpinfo['gdpfile'], gdpinfo['country_name'], gdpinfo['separator'], gdpinfo['quote'])
...
def render_xy_plot(gdpinfo, country_list, plot_file):
    ...
    #Function for viewing graphs in browser
